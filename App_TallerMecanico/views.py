...
def mecanico_panel(request):
    ...
    trabajos = Trabajo.objects.filter(mecanico__registro__id=request.session.get('user_id'))
...
def registrar_reparaciones(request, trabajo_id):
    ...
    # Paginación de reparaciones si es necesario
    paginator = Paginator(reparaciones, 5)  # 5 reparaciones por página
    page_number = request.GET.get('page')
